...
test("mock treatment keeps inactive tabs flat, lifts the active tab, and keeps containers hairline", () => {
  ...
  assert.match(styles, /\.tabs button\.active \{[\s\S]*?color: #e8f5ff;[\s\S]*?background: var\(--surface-strong\);[\s\S]*?border: 1px solid #16435f;[\s\S]*?border-bottom: 2px solid #17d9ff;[\s\S]*?calc\(100% - 6px\)[\s\S]*?box-shadow: inset 0 1px 0 rgb\(23 217 255 \/ 10%\);/);
  assert.match(styles, /\.stat-module \{[\s\S]*?background: var\(--surface-strong\);[\s\S]*?border: 1px solid #16435f;/);
  ...
  assert.match(styles, /\.stat-module \{[\s\S]*?height: 64px;[\s\S]*?background: var\(--surface-strong\);[\s\S]*?border: 1px solid #16435f;/);
